...
class BinarySearchTree
{
  public:
    ...
    int ipl() const {    // btw its also called indian premier league
        if(root == nullptr) 
            return 0;
    ...
    int getSize() const {
        // returns size of bst (number of nodes.)
...
*****************************************************/
    ...
    int ipl(const BinaryNode* root, const int depth = 0) const {
        ...
        if (root->left != NULL || root->right != NULL) {
            ...
            if (root->right != NULL) 
                count +=  ipl(root->right, depth+1);
        }
        return count;
    }

    int getSize(const BinaryNode *t) const {
        int count = 1;
        if(t->left != nullptr)
            count = count + getSize(t->left);
        if(t->right != nullptr)
            count = count + getSize(t->right);
        return count;
    }

    int getHeight(const BinaryNode* root) const {
        int height = 0;
